namespace Pluton.Rust
{
	...
	public class Hooks : Core.Hooks
	{
            ...
            if (!ConVar.Chat.enabled) {
                ...
                if (ce.FinalText != "") {
                    ...
                    if (ConVar.Server.globalchat) {
                        ConsoleNetwork.BroadcastToAllClients("chat.add2", basePlayer.userID, ce.FinalText, ce.BroadcastName, colour, 1);
                    }
                    else {
                        float num = 2500;
                        ...
                        foreach (BasePlayer current in BasePlayer.activePlayerList) {
                            ...
                            if (sqrMagnitude <= num) {
                                ConsoleNetwork.SendClientCommand(current.net.connection, "chat.add2", basePlayer.userID, ce.FinalText, ce.BroadcastName, colour, Mathf.Clamp01(num - sqrMagnitude + 0.2f));
                            }
                        }
                    }
                }
        ...
        public static void On_ItemPickup(CollectibleEntity entity, BaseEntity.RPCMessage msg)
        {
            ...
            msg.player.xp.Add(global::Rust.Xp.Definitions.CollectWorldItem, entity.xpScale, entity.ShortPrefabName, 0uL);
        ...
        public static void On_BuildingPartGradeChange(BuildingBlock block, BaseEntity.RPCMessage msg)
        {
            ...
            Effect.server.Run("assets/bundled/prefabs/fx/build/promote_" + bpgce.Grade.ToString().ToLower() + ".prefab", block, 0u, Vector3.zero, Vector3.zero);
        }
        ...
        public static void On_DoorUse(Door door, BaseEntity.RPCMessage msg, bool open)
        {
            ...
            if (!due.Allow) {
                if (due.DenyReason != "")
                    msg.player.SendConsoleCommand("chat.add", 0, String.Format("{0}: {1}", Server.server_message_name.ColorText("fa5"), due.DenyReason));
        ...
        public static void On_PlayerGathering(ResourceDispenser dispenser, BaseEntity to, ItemAmount itemAmount, float gatherDamage, float destroyFraction)
        {
